...
m=function(c){var d=c.touches?c.touches[0]:c,b=d.clientX;d=d.clientY;f&&f.valid&&(p={x:b-f.x,y:d-f.y},"undefined"===typeof q&&(q=!!(q||Math.abs(p.x)<Math.abs(p.y))),!q&&f&&(c.preventDefault(),y(a,e+p.x,0,null)))},t=function J(v){var k=g("#zuck-modal .viewing"),u=f;if(!f||f.valid){if(p){var x=f?Date.now()-f.time:void 0;x=300>Number(x)&&25<Math.abs(p.x)||Math.abs(p.x)>b.slideWidth/3;var C=0>p.x,t=C?g("#zuck-modal .story-viewer.next"):g("#zuck-modal .story-viewer.previous");q||(!x||C&&!t||!C&&!t?y(a,
e,300):c(C));f=void 0;a.removeEventListener("mousemove",m);a.removeEventListener("mouseup",J);a.removeEventListener("mouseleave",J);a.removeEventListener("touchmove",m);a.removeEventListener("touchend",J)}x=d.internalData.currentVideoElement;r&&clearInterval(r);k&&(E(k,k.querySelectorAll(".active"),!1),k.classList.remove("longPress"),k.classList.remove("paused"));if(l)if(clearInterval(l),l=!1,k=function(){C||(u.x>n.screen.width/3||!h("previousTap")?d.navigateItem("next",v):d.navigateItem("previous",
